...
class ArticleController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($request->category){
            ...
            $blogs=Article::where('status',1)->where('article_category_id','like','%'.$cat_id.'%')->orderby('title')->paginate(12);
        }else{
            $blogs=Article::where('status',1)->where('article_category_id','like','%'.$cat[0]->id.'%')->orderby('title')->paginate(12);
        }
