...
class ArticleRepository extends BaseRepository implements ArticleContract
{
    ...
    public function createArticle(array $params)
    {
        try {
            ...
            $article->blog_category_id = implode(',',$collection['article_category_id']);
            $article->blog_sub_category_id = $collection['article_sub_category_id'];
            $article->blog_tertiary_category_id = $collection['article_tertiary_category_id'];
            ...
            $article->heading = $collection['heading'] ?? '';
            $article->sticky_content = $collection['sticky_content'] ?? '';
            $article->btn_text = $collection['btn_text'] ?? '';
            $article->btn_link = $collection['btn_link'] ?? '';
            // slug generate
    ...
    public function updateArticle(array $params)
    {
        ...
        if(!empty($params['article_tertiary_category_id'])) {
        $article->blog_tertiary_category_id = $collection['article_tertiary_category_id'] ?? '';
        }
        if(!empty($params['pincode'])) {
        $article->pincode = $collection['pincode'] ?? '';
        }
        if(!empty($params['suburb_id'])) {
        $article->suburb_id = $collection['suburb_id'] ?? '';
        }
        $article->content = $collection['content'];
        $article->meta_title = $collection['meta_title'] ?? '';
        $article->meta_key = $collection['meta_key'] ?? '';
        ...
        if(!empty($params['image'])) {
           ...
           if(!empty($params['sticky_image'])){
            ...
            $profile_image->move("Blogs/",$imageName);
            $uploadedImage = $imageName;
            $article->sticky_image = $uploadedImage;
            }
        $article->save();
    ...
    public function getArticletertiarycategories(){
        $categories = ArticleTertiaryCategory::orderBy('title')->get();
        return $categories;
    }
