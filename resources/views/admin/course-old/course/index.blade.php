    ...
    <div class="app-title">
        <div class="row w-100">
            ...
            <div class="col-md-6 text-right">
                <a href="{{ route('admin.course.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i> Add New</a>
                  ...
                  <div class="col-md-3">
                      <select class="filter_select form-control" name="category">
                          ...
                          @foreach ($categories as $index => $item)
                               <option value="{{$item->title}}" {{ (request()->input('category') == $item->title) ? 'selected' : '' }}>{{ $item->title }}</option>
                          @endforeach
                      </select>
                  </div>
                  <div class="col-md-3">
                    <input type="text" name="keyword" id="keyword" class="form-control" placeholder="Course Name" value="{{app('request')->input('keyword')}}" autocomplete="off">
                  </div>
                  <div class="col-md-3">
                      <input type="text" name="author" id="author" class="form-control" placeholder="Author Name" value="{{app('request')->input('author')}}" autocomplete="off">
                  </div>
                  <div class="col-md-3">
                    <select name="type" id="type" class="form-control @error('type') is-invalid @enderror">
                        <option value="">Select an option</option>
                        <option value="1" {{ (1 == request()->input('type')) ? 'selected' : ''  }}>Paid</option>
                        <option value="2" {{ (2 == request()->input('type')) ? 'selected' : ''  }}>Free</option>
                    </select>
                </div>
              </div>
              ...
              <div class="row align-items-center justify-content-between">
                  <div class="col">
                      <ul class="text-right mt-3">
                          <p class="font-weight : bold">Total Events <span class="count">({{$course->total()}})</span></p>
                      </ul>
                  </div>
              </div>
              <div class="row">
                        ...
                        <tbody>
                            @foreach($course as $key => $data)
                                <tr>
                                    ...
                                    <td>
                                        @if($data->image!='')
                                        <img style="width: 100px;height: 100px;" src="{{URL::to('/').'/course/'}}{{$data->image}}">
                                        @endif
                                    </td>
                                    <td>{{$data->category ? $data->category->title : '' }}</td>
                                    <td>{{ $data->course_name }}</td>
                                    <td>{{ $data->author_name }}</td>
                                    <td>{{ ($data->type==1)? 'Paid':'Free' }}</td>
                                ...
                                <td class="text-center">
                                    <div class="btn-group" role="group" aria-label="Second group">
                                        <a href="{{ route('admin.course.edit', $data['id']) }}" class="btn btn-sm btn-primary edit-btn"><i class="fa fa-edit"></i></a>
                                        <a href="{{ route('admin.course.details', $data['id']) }}" class="btn btn-sm btn-primary edit-btn"><i class="fa fa-eye"></i></a>
                ...
                <div class="modal-body">
                    <form method="post" action="{{route('admin.course.data.csv.store')}}" enctype="multipart/form-data" id="fileCsvUploadForm">
                        ...
                        <button type="submit" class="btn btn-sm btn-primary" id="csvImportBtn">Import <i class="fas fa-upload"></i></button>
                         <p><a href="{{URL::to('/')}}/admin/csvexample/course.csv" target="_blank">
...
@push('scripts')
    ...
    $('.sa-remove').on("click",function(){
        ...
        function(isConfirm){
          if (isConfirm) {
            window.location.href = "course/"+id+"/delete";
            } else {
