    ...
    <div class="app-title">
        <div class="row w-100">
            ...
            <div class="col-md-6 text-right">
                <a href="{{ route('admin.tools.AreaOfInterest.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i> Add New</a>
            </div>
        </div>
    </div>
            ...
            <div class="row align-items-center justify-content-between">
                <div class="col">
                    <ul class="text-right mt-3">
                        <p class="font-weight : bold">Total <span class="count">({{$data->total()}})</span></p>
                    </ul>
                </div>
                <div class="col-auto">
                    <form action="{{ route('admin.tools.AreaOfInterest.index') }}">
                            ...
                            @foreach($data as $key => $category)
                                ...
                                <td class="text-center">
                                    <div class="btn-group" role="group" aria-label="Second group">
                                        <a href="{{ route('admin.tools.AreaOfInterest.edit', $category['id']) }}" class="btn btn-sm btn-primary edit-btn"><i class="fa fa-edit"></i></a>
                                        <a href="{{ route('admin.tools.AreaOfInterest.details', $category['id']) }}" class="btn btn-sm btn-primary edit-btn"><i class="fa fa-eye"></i></a>
...
@push('scripts')
    ...
    $('.sa-remove').on("click",function(){
        ...
        function(isConfirm){
          if (isConfirm) {
            window.location.href = "AreaOfInterest/"+categoryid+"/delete";
            } else {
    ...
    <script type="text/javascript">
        $('input[id="toggle-block"]').change(function() {
          ...
          $.ajax({
                type:'POST',
                dataType:'JSON',
                url:"{{route('admin.tools.AreaOfInterest.updateStatus')}}",
